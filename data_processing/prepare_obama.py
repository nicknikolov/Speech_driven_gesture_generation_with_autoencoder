...
def _download_datasets(data_dir):
    ...
    for i in range (FIRST_DATA_ID, LAST_DATA_ID - NUM_OF_TEST):
        ...
        original_file_path = path.join("obama/speech/" + filename)
        ...
        original_file_path = path.join("obama/headpose/" + filename)
        if os.path.exists(original_file_path):
...
def _files_to_pandas_dataframe(extracted_dir, set_name, idx_range):
    ...
    return pandas.DataFrame(data=files, columns=["wav_filename", "wav_filesize", "pose_filename"])
